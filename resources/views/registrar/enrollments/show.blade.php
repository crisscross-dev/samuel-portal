...
<div class="row g-4">
    ...
    <div class="col-md-5">
        <div class="card mb-3">
            <div class="card-header"><h6 class="mb-0">Enrollment Info</h6></div>
            <div class="card-body">
                <table class="table table-borderless mb-0">
                    <tr><th class="text-muted" style="width:40%">Student</th><td>{{ $enrollment->student->user->name ?? 'N/A' }}</td></tr>
                    <tr><th class="text-muted">Student ID</th><td>{{ $enrollment->student->student_id ?? '' }}</td></tr>
                    <tr><th class="text-muted">Program</th><td>{{ $enrollment->student->program->code ?? 'N/A' }}</td></tr>
                    <tr><th class="text-muted">Section</th><td>{{ $enrollment->student->section?->displayName() ?? 'N/A' }}</td></tr>
                    <tr><th class="text-muted">Semester</th><td>{{ $enrollment->semester->academicYear->name ?? '' }} - {{ $enrollment->semester->name ?? '' }}</td></tr>
                    ...
                    <tr><th class="text-muted">Units</th><td>{{ $enrollment->total_units }}</td></tr>
                    <tr><th class="text-muted">Total Amount</th><td>&#8369;{{ number_format($enrollment->total_amount, 2) }}</td></tr>
                    @if($enrollment->enrolled_at)
                        <tr><th class="text-muted">Enrolled At</th><td>{{ $enrollment->enrolled_at->format('M d, Y h:i A') }}</td></tr>
                    @endif
                </table>

                <div class="d-flex gap-2 mt-3">
                    @if($enrollment->status === 'pending')
                        <form action="{{ route('registrar.enrollments.assess', $enrollment) }}" method="POST">
                            @csrf
                            <button class="btn btn-info btn-sm"><i class="bi bi-calculator me-1"></i> Generate Assessment</button>
                        </form>
                    @endif
        ...
        {{-- Subjects --}}
        <div class="card">
            <div class="card-header"><h6 class="mb-0">Enrolled Subjects</h6></div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-sm mb-0">
                        <thead class="table-light">
                            @if(!empty($breakdown) && $breakdown['pricing_type'] === 'per_unit')
                                <tr><th>Code</th><th>Subject</th><th>Section</th><th class="text-center">Lec</th><th class="text-center">Lab</th><th class="text-center">Total</th></tr>
                            @else
                                <tr><th>Code</th><th>Subject</th><th>Section</th><th class="text-center">Units</th></tr>
                            @endif
                        </thead>
                        <tbody>
    ...
    <div class="col-md-7">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                @if(in_array($enrollment->status, ['assessed', 'enrolled']))
                    <a href="{{ route('registrar.payments.create', ['enrollment_id' => $enrollment->id]) }}" class="btn btn-sm btn-primary">
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-sm mb-0">
                        <thead class="table-light">
                            <tr><th>Date</th><th>Amount</th><th>Method</th><th>Ref#</th><th>Status</th><th>Actions</th></tr>
                        </thead>
                        <tbody>
