    ...
    <div class="col-md-2">
        <div class="card">
            <div class="card-body">
                ...
                <div class="fs-4 fw-bold text-info">{{ $stats['exam_approved'] }}</div>
            </div>
        </div>
    </div>
    <div class="col-md-2">
        <div class="card">
            <div class="card-body">
                ...
                <div class="fs-4 fw-bold text-primary">{{ $stats['guidance_review'] }}</div>
            </div>
        </div>
    </div>
    <div class="col-md-2">
        <div class="card">
            <div class="card-body">
                <div class="text-muted small">Requirements Check</div>
                <div class="fs-4 fw-bold text-primary">{{ $stats['registrar_requirements'] }}</div>
            </div>
        </div>
    </div>
    <div class="col-md-2">
        <div class="card">
            <div class="card-body">
                <div class="text-muted small">Enrollment Stage</div>
                <div class="fs-4 fw-bold text-success">{{ $stats['enrollment'] }}</div>
            </div>
        </div>
    </div>
    <div class="col-md-2">
        <div class="card">
            <div class="card-body">
                ...
                <div class="fs-4 fw-bold text-dark">{{ $stats['cashier'] }}</div>
                <div class="small text-danger mt-1">Inactive: {{ $stats['inactive'] }}</div>
            </div>
        </div>
    </div>
</div>
...
<div class="card mb-4 border-0 shadow-sm">
    <div class="card-body d-flex flex-column flex-lg-row align-items-start align-items-lg-center justify-content-between gap-3">
        <div>
            <div class="text-muted small text-uppercase fw-semibold">Applications Workflow</div>
            <h5 class="mb-1">{{ $currentQueue['title'] }}</h5>
            <p class="text-muted mb-0 small">{{ $currentQueue['description'] }}</p>
        </div>
        <div class="d-flex flex-wrap gap-2">
            <a href="{{ route('registrar.applications.index', ['stage_group' => 'admission']) }}" class="btn btn-sm {{ $selectedStageGroup === 'admission' ? 'btn-primary' : 'btn-outline-primary' }}">Admission</a>
            <a href="{{ route('registrar.applications.index', ['stage_group' => 'exam']) }}" class="btn btn-sm {{ $selectedStageGroup === 'exam' ? 'btn-primary' : 'btn-outline-primary' }}">Exam</a>
            <a href="{{ route('registrar.applications.index', ['stage_group' => 'requirements']) }}" class="btn btn-sm {{ $selectedStageGroup === 'requirements' ? 'btn-primary' : 'btn-outline-primary' }}">Requirements</a>
            <a href="{{ route('registrar.applications.index') }}" class="btn btn-sm {{ $selectedStageGroup === 'all' ? 'btn-secondary' : 'btn-outline-secondary' }}">All</a>
        </div>
    </div>
</div>
