    ...
    <div class="card-body border-bottom pb-3 mb-0">
        <form method="GET" class="row g-2">
            ...
            @if(request()->hasAny(['department_id','academic_year_id']))
                <div class="col-md-2">
                    <a href="{{ route('admin.pricing.index') }}" class="btn btn-sm btn-outline-secondary w-100">Clear</a>
                </div>
            @endif
        </form>
    </div>
                ...
                <tbody>
                        ...
                        <tr class="{{ !$ts->is_active ? 'table-secondary' : '' }}">
                            ...
                            <td>
                                <div class="d-flex gap-1">
                                    <a href="{{ route('admin.pricing.show', $ts) }}" class="btn btn-sm btn-outline-info" title="View"><i class="bi bi-eye"></i></a>
                                    ...
                                    <form method="POST" action="{{ route('admin.pricing.toggle-active', $ts) }}" class="d-inline">
                                        ...
                                        <button class="btn btn-sm btn-outline-{{ $ts->is_active ? 'secondary' : 'success' }}"
                                            ...
                                            onclick="return confirm('{{ $ts->is_active ? 'Deactivate' : 'Activate' }} this structure?')">
                                    ...
                                    <form method="POST" action="{{ route('admin.pricing.destroy', $ts) }}" class="d-inline"
                                        onsubmit="return confirm('Delete this pricing structure? This cannot be undone.')">
                                        @csrf @method('DELETE')
                    ...
                    @empty
                        <tr><td colspan="8" class="text-center text-muted py-4">
                            ...
                            No tuition structures found. <a href="{{ route('admin.pricing.create') }}">Create one.</a>
                        </td></tr>
                    @endforelse
                </tbody>
            </table>
